...
public class Panel_BasicTemplate extends ApplicationWindow {
	...
	protected Control createContents(Composite parent) {
		...
		btn_submit.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				if (text_tempName.getText() == null || text_tempName.getText().trim().length() == 0) {
				...
				tempName = text_tempName.getText().trim();
				tempType = combo_tempType.getText().trim();
				validTime = text_validTime.getText().trim();
				unit = combo_unit.getText().trim();
				rsaMinLen = combo_rsaMinLen.getText().trim();
				signAlg = combo_signAlg.getText().trim();
				close();
	...
	public void tempAppendBasicInfo(CertTemplate certTemplate) {
		...
		certTemplate.setCerttemplateName(text_tempName.getText().trim());
		certTemplate.setSpecialType(combo_tempType.getText());
		certTemplate.setValiditylimit(Integer.valueOf(text_validTime.getText().trim()));
