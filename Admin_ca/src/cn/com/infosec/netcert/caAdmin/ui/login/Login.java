...
public class Login extends ApplicationWindow {
	...
	private String keyDriverName, keyDriverLibPath, subject;
	...
	public Login() {
		super(null);
		setShellStyle(SWT.MIN | SWT.CLOSE); // 支持最小化 和 拖动
	...
	protected Control createContents(Composite parent) {
		...
		if (ALG.SM2 == Env.alg) { // dev名称 dev_SN
			...
			combo_skf_devSN.setEnabled(true);
		...
		btn_login.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (ALG.SM2 == Env.alg
						&& (combo_skf_devSN.getText() == null || combo_skf_devSN.getText().trim().length() == 0)) {
					MessageBox mb = new MessageBox(getShell());
				...
				if (ALG.SM2 == Env.alg) {
					try {
						...
						//Env.getMap().put(pro.getProperty(combo_driverName.getText()) + combo_skf_devSN.getText(), skf);
				...
				try {
					...
					if ("SA".equalsIgnoreCase(role) || "AA".equalsIgnoreCase(role)) {
						...
						BO bo = new BO(subject, resp.getProperty(PropertiesKeysRes.ADMIN_BO_POWERS));
					...
					log.errlog("", se);
					String message = l.getString("Notice_fail_login") + ":" + "[" + se.getErrorNum() + "]:"
							+ se.getErrorMsg();
					Panel_MessageDialog dialog = new Panel_MessageDialog("error", message);
					dialog.setBlockOnOpen(true);
					dialog.open();
					btn_login.setEnabled(true);
				} catch (Exception ee) {
	...
	protected void configureShell(Shell shell) {
		...
		shell.setText("Infosec CA-Admin");
