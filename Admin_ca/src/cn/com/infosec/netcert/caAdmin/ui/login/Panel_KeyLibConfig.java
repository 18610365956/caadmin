...
public class Panel_KeyLibConfig extends ApplicationWindow {
	...
	public Panel_KeyLibConfig() {
		...
		try {
			...
			log.errlog("Create file fail/load configFile fail", e);
	...
	protected Control createContents(Composite parent) {
		...
		Label lbl_customName = new Label(container, SWT.NONE);
		GridData gd_lbl_customName = new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1);
		gd_lbl_customName.widthHint = 85;
		lbl_customName.setLayoutData(gd_lbl_customName);
		lbl_customName.setAlignment(SWT.RIGHT);
		lbl_customName.setText(l.getString("customName") + ":");
		...
		lbl_USBKey.setText(l.getString("driverFile") + ":");
		...
		btn_bind.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				if (keyFile == null || keyFile.length() == 0) {
					MessageBox mb = new MessageBox(getShell(), SWT.ERROR);
					mb.setMessage(l.getString("Notice_null_driverFile"));
					mb.open();
					return;
				}
				...
				try {
					...
					log.errlog("file not found/configFile load fail.", e2);
					MessageBox mb = new MessageBox(getShell(), SWT.ERROR);
		...
		btn_unBind.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				if (value != null) {
					...
					MessageBox mb = new MessageBox(getShell(), SWT.ERROR);
					mb.setMessage(l.getString("Notice_notExist_customName"));
					mb.open();
				}
			}
		});
