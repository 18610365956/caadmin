...
public class AdminClient {
	 ...
	 * @param chSign   请求签名配置
	 * @param chSecure 通讯安全配置
	 * @param chConf  通讯基本设置，ip、端口等
	 * @throws Exception
	 */
	public AdminClient(ChannelSign chSign, ChannelSecure chSecure, String protocolName, ChannelConf chConf)
		...
		if (chSign != null) { 
			if (ALG.SM2 == Env.alg) {
				...
				//System.out.println("new:" + ((UsbKeySKFImpl)hsm).getHApp() +","+ ((UsbKeySKFImpl)hsm).getHDev());  // 测试 中标环境 写用户证书失败
		...
		if (!this.isAnonymous) {
			...
			if (ALG.SM2 == Env.alg) {
				...
				try {
					...
					// System.out.println("sign:" + ((UsbKeySKFImpl)hsm).getHApp() +","+ ((UsbKeySKFImpl)hsm).getHDev()); 测试 中标麒麟写用户单证书失败
