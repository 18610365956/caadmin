...
public class Env {
	...
	public static boolean IS_USER_AUTH_BY_CA = true;
		...
		if (!f.exists()) {
			...
			if (caCer != null && caCer.length() > 0) {
				...
				if (fca.exists()) {
					...
					fin.close();
					if (!trustCert.getSubjectDN().getName().equalsIgnoreCase(trustCert.getIssuerDN().getName())) {
						throw new Exception(l.getString("Notice_not_rootCert"));
					}
					caCerFile = caCer;
					...
					Env.alg = ALG.SM2;
					throw new Exception(l.getString("Notice_null_cert"));
				}
			}
		}
	}
